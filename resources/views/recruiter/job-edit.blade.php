...
@section('content')
    <div class="space-y-6">
        <section class="rounded-[28px] border border-slate-200 bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 p-8 text-white shadow-lg shadow-slate-200/60">
            ...
            <h1 class="mt-3 text-4xl font-black tracking-tight">Modifier l’offre</h1>
            <p class="mt-3 max-w-2xl text-slate-200">Mettez à jour le poste, les conditions et la visibilité pour rester aligné avec vos besoins de recrutement.</p>
        </section>

        <div class="card mx-auto max-w-5xl">
            <form method="POST" action="{{ route('recruiter.job.update', $jobOffer) }}" class="grid gap-5 md:grid-cols-2">
                @csrf
                @method('PUT')

                <div>
                    <label for="company" class="mb-2 block text-sm font-semibold text-slate-700">Entreprise</label>
                    <input id="company" type="text" name="company" value="{{ old('company', $jobOffer->company) }}" class="input-field" required>
                </div>

                <div>
                    <label for="title" class="mb-2 block text-sm font-semibold text-slate-700">Intitulé du poste</label>
                    <input id="title" type="text" name="title" value="{{ old('title', $jobOffer->title) }}" class="input-field" required>
                </div>

                <div>
                    <label for="location" class="mb-2 block text-sm font-semibold text-slate-700">Localisation</label>
                    <input id="location" type="text" name="location" value="{{ old('location', $jobOffer->location) }}" class="input-field" required>
                </div>

                <div>
                    <label for="city" class="mb-2 block text-sm font-semibold text-slate-700">Ville</label>
                    <input id="city" type="text" name="city" value="{{ old('city', $jobOffer->city) }}" class="input-field">
                </div>
                ...
                <div>
                    ...
                    <input id="domain" type="text" name="domain" value="{{ old('domain', $jobOffer->domain) }}" class="input-field" placeholder="IT, Marketing, Finance">
                </div>
                ...
                <div>
                    ...
                    <input id="salary_min" type="number" step="0.01" name="salary_min" value="{{ old('salary_min', $jobOffer->salary_min) }}" class="input-field">
                </div>

                <div>
                    <label for="salary_max" class="mb-2 block text-sm font-semibold text-slate-700">Salaire maximum</label>
                    <input id="salary_max" type="number" step="0.01" name="salary_max" value="{{ old('salary_max', $jobOffer->salary_max) }}" class="input-field">
                </div>

                <div>
                    <label for="required_experience" class="mb-2 block text-sm font-semibold text-slate-700">Expérience requise</label>
                    <input id="required_experience" type="text" name="required_experience" value="{{ old('required_experience', $jobOffer->required_experience) }}" class="input-field" placeholder="3+ ans, Senior, Junior">
                </div>
                ...
                <div>
                    ...
                    <input id="deadline" type="date" name="deadline" value="{{ old('deadline', $jobOffer->deadline ? $jobOffer->deadline->format('Y-m-d') : '') }}" class="input-field">
                </div>

                <div class="md:col-span-2">
                    <label for="description" class="mb-2 block text-sm font-semibold text-slate-700">Description</label>
                    <textarea id="description" name="description" rows="6" class="input-field" required>{{ old('description', $jobOffer->description) }}</textarea>
                </div>

                <div>
                    <label for="required_skills" class="mb-2 block text-sm font-semibold text-slate-700">Compétences</label>
                    <input id="required_skills" type="text" name="required_skills" value="{{ old('required_skills', implode(', ', json_decode($jobOffer->required_skills ?? '[]', true) ?: [])) }}" class="input-field">
                </div>

                <div>
                    <label for="benefits" class="mb-2 block text-sm font-semibold text-slate-700">Avantages</label>
                    <input id="benefits" type="text" name="benefits" value="{{ old('benefits', implode(', ', json_decode($jobOffer->benefits ?? '[]', true) ?: [])) }}" class="input-field">
                </div>
